		 ...
		 double theta, double delta, int *nit, int *nf, double *xl, double *xr, int *term)
{
	...
	*nf = 0;
	...
	for (i = 0; i < n + 1; i++)
	{
#pragma omp task firstprivate(i) shared(u, fu, n)
		{
			fu[i] = f(&u[i * n], n);
#pragma omp atomic
			*nf = *nf + 1;
...
#pragma omp task firstprivate(i) shared(r, fr, n)
					{
						fr[i] = f(&r[i * n], n);
#pragma omp atomic
						*nf = *nf + 1;
...
#pragma omp task firstprivate(i) shared(u, ec, fec, n, mu)
						{
							fec[i] = f(&ec[i * n], n);
#pragma omp atomic
							*nf = *nf + 1;
...
#pragma omp task firstprivate(i) shared(u, ec, fec, n, theta)
					{
						fec[i] = f(&ec[i * n], n);
#pragma omp atomic
						*nf = *nf + 1;
