...
/* User-defined reduction for TrialResult: keep the one with smaller fx */
#pragma omp declare reduction(best_red:TrialResult : omp_out = (omp_in.fx < omp_out.fx ? omp_in : omp_out)) \
	initializer(omp_priv = {1e10, -1, -1, -1, {0}})
...
/* trial doesn't need to be declared but it is declared in order to avoid ambiguity*/
#pragma omp parallel for private(trial, i, startpt, endpt, fx, nt, nf) reduction(best_red : best) schedule(dynamic)
