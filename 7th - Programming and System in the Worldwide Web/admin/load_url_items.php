...
if ($url) {
    ...
    if ($json === false) {
        echo json_encode(['success' => false, 'message' => 'Failed to fetch JSON from URL']);
        return;
    }
    ...
    if (isset($data['items']) && isset($data['categories'])) {
        processJsonData($data, $conn); // Process the JSON data
    } else {
        echo json_encode(['success' => false, 'message' => 'Invalid JSON structure from URL']);
    }
} else {
